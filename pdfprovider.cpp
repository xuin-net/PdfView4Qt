...
QString PdfProvider::getFilePath()
{
    return filenamepath;
}
...
void PdfProvider::loadFile(const QString filePath, QVariant width,QVariant height, QVariant fitMode)
{
    if(dropIMG) {
    ...
    filenamepath = std::move(filePath);
    QByteArray inBytes;
    ...
    setDefaultwidth(width.toInt());
    setDefaultheight(height.toInt());
    setFitToOption(fitMode.toInt());
    emit pageCntChanged();
